...
<head>
    ...
    <link rel="stylesheet" href="{{asset('otp/css/otpinbox.css')}}" />
...
<section class="w-100 h-100 ">
    <div class="otpverify">
        <div class="otpinboxicon">
            <img src="{{asset('otp/img/Group 78.svg')}}" alt="otpinbox" />
        </div>
        <div class="otpContent">
            <h5>Enter the OTP sent on your mail</h5>
            <p>
                Enter the OTP that has been sent on your email ID
                {{$tempuser->email}}
            </p>
            <form  action = "{{route('login.otp.verify')}}" id="otp_verify_form"  method="POST">
                @csrf
                ...
                <div class="text-center">
                        <a href="{{route('resend.otp',['username' => $tempuser->email,'type' => 'email','user_type'=>'user','user_id' => $tempuser->id])}}" class="text-primary text-center">Resend OTP</a>
                </div>
                <input type="hidden" name="otpnumber" class="otpnumber"/>
                <input type="hidden" name="temp_user_id" value="{{$tempuser->id}}"/>
                <button class="hide otpbutton" id="submit" onclick="validateOTP()">Submit</button>
